...
fun main() {
    ...
    val initialMomentum = generatePointInsideUnitCircle() * 10
...
private fun calcAndDraw(initialPoint: Point, initialMomentum: Momentum) {
    ...
    repeat(N) {
        val nextPoint = point + momentum + g
...
private fun drawLines(lines: List<Line>, color: String): geomSegment {
    ...
    return geomSegment(data = dataPoints, color = color) {
        x = "startX"
        y = "startY"
        xend = "endX"
        yend = "endY"
